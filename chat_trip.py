...
st.set_page_config(page_title="여행 계획 챗봇", layout="wide", initial_sidebar_state="expanded")
...
st.markdown(
    ...
    /* 전체 배경색 */
    .stApp {
        background-color: #F0F2F6; /* 은은한 회색 계열 */
    ...
    h2 {
        color: #34495E; /* 조금 더 밝은 남색 */
        font-size: 2em;
        border-bottom: 2px solid #BDC3C7; /* 하단 경계선 */
    ...
    /* 사이드바 스타일 */
    .stSidebar {
        background-color: #FFFFFF; /* 사이드바 배경을 흰색으로 */
        color: #333333;
    ...
    .stSidebar .stButton>button {
        ...
        background-color: #ECF0F1; /* 버튼 배경색 */
        color: #2C3E50;
    }
    .stSidebar .stButton>button:hover {
        background-color: #BDC3C7; /* 호버시 색상 */
        color: #FFFFFF;
    }
    ...
    .stButton>button:hover {
        background-color: #217DBB; /* 호버 시 더 진하게 */
    ...
    .stAlert.info {
        background-color: #D1ECF1; /* 연한 파란색 */
        color: #0C5460;
        border-color: #BEE5EB;
    }
    ...
    .st-ag .ag-row-even {
        background-color: #F8F9FA !important; /* 짝수 행 배경색 */
    }
    .st-ag .ag-row-odd {
        background-color: #FFFFFF !important; /* 홀수 행 배경색 */
    }
...
if __name__ == "__main__":
    ...
    if st.session_state.selected_conversation_index is not None:
        ...
        if st.button("🚀 여행 계획 추천받기", use_container_width=True):
            st.session_state.selected_conversation_index = None 
            ...
            if lat_to_invoke is None or lon_to_invoke is None:
                st.warning("⚠️ 위치 정보가 없으므로 답변을 생성할 수 없습니다. 위치 정보를 입력하거나 가져와 주세요.")
            elif not user_query.strip():
                st.warning("⚠️ 질문을 입력해주세요.")
            else:
                with st.spinner("⏳ 최적의 여행 계획을 수립 중입니다... 잠시만 기다려 주세요!"):
                    try:
